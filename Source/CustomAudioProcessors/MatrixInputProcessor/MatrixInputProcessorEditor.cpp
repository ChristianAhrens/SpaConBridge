...
{
	MatrixInputProcessor* mcProcessor = dynamic_cast<MatrixInputProcessor*>(getAudioProcessor());
	if (mcProcessor)
	{
	...
	//
	//	mcProcessor->SetParameterValue(DCS_MatrixInputProcessor, paramIdx, static_cast<float>(slider->getValue()));
	}
}
 ...
 */
void MatrixInputProcessorEditor::textEditorReturnKeyPressed(TextEditor& textEditor)
{
...
void MatrixInputProcessorEditor::UpdateGui(bool init)
{
	...
	if (pro)
	{
		...
		if (pro->PopParameterChanged(DCS_MatrixInputProcessor, DCT_MatrixInputLevelMeter))
		{
			// Update level meter.
		...
		if (pro->PopParameterChanged(DCS_MatrixInputProcessor, DCT_MatrixInputGain))
		{
			// Update gain slider
		...
		if (pro->PopParameterChanged(DCS_MatrixInputProcessor, DCT_MatrixInputMute))
		{
			// Update mute button
		...
		if (pro->PopParameterChanged(DCS_MatrixInputProcessor, DCT_MatrixInputID))
		{
