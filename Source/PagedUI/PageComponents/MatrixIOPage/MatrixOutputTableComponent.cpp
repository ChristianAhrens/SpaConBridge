...
{
	auto w = std::make_unique<AlertWindow>("Matrix Outputs", "Choose how many to add", juce::AlertWindow::NoIcon).release();
	...
	auto callbackFunctionBody = ([w](int result)
		{
			if (result == 1)
			{
				...
				if (ctrl)
				{
					...
					if (processorCount > 0)
						ctrl->createNewMatrixOutputProcessors(processorCount);
				}
			}
		});
