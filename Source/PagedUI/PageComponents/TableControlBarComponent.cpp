	...
	: Component(componentName)
{
	...
	m_batchAddInstances = std::make_unique<DrawableButton>("add_batch", DrawableButton::ButtonStyle::ImageOnButtonBackground);
	m_batchAddInstances->setClickingTogglesState(false);
	m_batchAddInstances->addListener(this);
	m_batchAddInstances->setTooltip("Add multiple rows");
	addAndMakeVisible(m_batchAddInstances.get());
...
void TableControlBarComponent::lookAndFeelChanged()
{
	...
	if (dblookAndFeel)
	{
		...
		m_batchAddInstances->setImages(NormalImage.get(), OverImage.get(), DownImage.get(), DisabledImage.get(), NormalOnImage.get(), OverOnImage.get(), DownOnImage.get(), DisabledOnImage.get());
...
void TableControlBarComponent::buttonClicked(Button* button)
{
	...
	else if (button == m_batchAddInstances.get())
	{
		if (onBatchAddClick)
			onBatchAddClick();
	}
...
void TableControlBarComponent::resized()
{
	...
	if (m_layoutDirection == LD_Horizontal)
	{
		...
		mainFB.items.addArray({
			FlexItem(*m_batchAddInstances.get()).withWidth(25).withMargin(FlexItem::Margin(2, 2, 3, 2)),
	...
	else if (m_layoutDirection == LD_Vertical)
	{
		...
		mainFB.items.addArray({
			FlexItem(*m_batchAddInstances.get()).withHeight(25).withMargin(FlexItem::Margin(2, 2, 2, 3)),
